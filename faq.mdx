...
<AccordionGroup>
  ...
  <Accordion title="Where is data stored?">
    Self-hosting gives you complete control over data location. Nvisy is open-source, so the entire platform runs inside your own infrastructure — and pairing it with an `ollama` connection keeps inference local too, so no document content leaves your network.
    
    [On-premise deployment →](/deployment/on-premise/requirements)
  </Accordion>
  
  <Accordion title="Do you have access to my documents?">
    With self-hosted deployment, your documents never leave your infrastructure. Retention is yours to set per workspace and override per pipeline, including `zeroDays` to delete data as soon as processing finishes.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Why is redaction taking so long?">
    ...
    Processing is usually under 10 seconds. Large documents and OCR-heavy scans take longer.
  </Accordion>
